wcKMedoids <- function(diss, k, weights=NULL, npass = 1, initialclust=NULL, method="PAMonce", cluster.only = FALSE, debuglevel=0) {
	...
	ret <- .Call("RKmedoids", as.integer(nelements),  diss, quote(InternalRandomSample()), environment(), as.integer(initialclust), as.integer(npass), as.double(weights), as.integer(method), as.integer(debuglevel), as.integer(isdist))
