...
We may include several covariates by building a sequence regression trees with the \code{seqtree} function \citep{StuderRitschardGabadinhoMuller2011SMR}.\footnote{The previous approach can also be extended to the multifactor case.}
...
\section{Construction of the \texttt{test} variable}
