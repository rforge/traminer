...
\SweaveOpts{message=FALSE, prompt=TRUE, echo=TRUE, comment=NA, crop=TRUE, small.mar=TRUE, fig.align="center", fig.path="graphics/WC-", out.width=".8\\linewidth"} % sets the prefix for figure files generated par Sweave
...
\end{center}
Studer, Matthias (2012).\textit{Étude des inégalités de genre en début de carrière académique
à l’aide de méthodes innovatrices d’analyse de données séquentielles}, Chapitre: Le
manuel de la librairie WeightedCluster : Un guide pratique pour la création de typologies de trajectoires en sciences sociales avec R. Thèse de doctorat inédite, Université de Genève.
...
<<seqtreedisplay, echo=FALSE, results="hide">>=
...
<<wcClusterQuality-compute9-option, echo=FALSE, results="hide">>=
options(digits=2)
@
<<wcClusterQuality-compute9>>=
clustqual4 <- wcClusterQuality(mvaddist, clust4, weights=mvad$weight)
clustqual4$stats
@
<<wcClusterQuality-compute9-optionend, echo=FALSE, results="hide">>=
options(digits=3)
@


Selon la mesure \code{ASW=\Sexpr{round(clustqual4$stats["ASW"], 2)}}, la solution en quatre groupes obtenue avec le critère de Ward pourrait être un artifice statistique, puisqu'elle est inférieure à 0.25.


L'élément \code{ASW} de l'objet \code{clustqual4} contient la silhouette moyenne de chaque groupe pris séparément. Selon cette mesure, le groupe 3 est ci particulièrement mal défini puisque sa silhouette moyenne est négative.
...
%<<silhouette-indexplot, echo=FALSE, results="hide">>=
%sil <- wcSilhouetteObs(mvaddist, clust4, weights=mvad$weight)
%png(file=paste(graphdir,"png_mvad_seqIplot-sil.png",sep=""), unit="px", width=1600, height=1000, pointsize=30)
%par(mar=c(2.1, 4.1, 4.1, 1.1))
%seqIplot(mvadseq, group=clust4, sortv=sil)
%dev.off()
%@
%
%\includegraphics{png_mvad_seqIplot-sil}
...
mvad$pam4 <- factor(pamclust4$clustering, levels=c(66, 467, 607, 641), labels=c("Appr.-Emploi", "Ecole-Emploi", "Ecole sup.", "Sans emploi"))
table(mvad$pam4)
@
...
%
%
%\section{Algorithmes K-Medoïds}
%
%K-Medoïds algorithms aim at finding the best partition of the data in a $k$ predefined number of groups. Based on a dissimilarity matrix, those algorithms seeks to minimize the (weighted) sum of distance to the medoïd of each group. The medoïd is defined as the observation that minimize the sum of distance to the other observations of this group.
%
%The \pkg{WeightedCluster} library provides two different algorithms: k-medoïd~\citep{Hoon2010} and PAM~\citep{RClusterPackage, StruyfHubertRousseeuw1997}. The K-Medoïd use the following strategy:
%
%\begin{enumerate}
%    \item Take $k$ random observations as the starting medoïds.
%    \item Assign each observations to the closest medoïd.
%    \item Recompute the new medoïd of each group defined in the previous step.
%    \item Repeat step (2) et (3) until a stable solution is found.
%\end{enumerate}
%
%This algorithm may be attracted by a local minimum and the solution depend on the original medoïds used. For this reason, it is usually recommended to launch the algorithm several times and to keep the best solution found.


%
%L'algorithme peut tendre vers un minimum local et la partition identifiée peut varier en fortement selon les médoïdes choisis au départ. Pour cette raison, il est conseillé de relancer la procédure un grand nombre de fois et de retenir la meilleure solution~\citep{Hoon2010}.
...
EcoleEmploi <- mvad$pam4=="Ecole-Emploi"
...
uniqueSec <- seqdef(mvad[ac$aggIndex, 17:86], weights=ac$aggWeights)
@
...
mvaddist2 <- seqdist(uniqueSec, method="OM", indel=1.5, sm=subm.custom)
...
\begin{eqnarray}
    a_i&=&\frac{1}{W_k-w_i}\sum_{j \in k} w_j d_{ij}\label{eq_clustqual_asw_ai}\\
    b_i&=&\min_{\ell} \frac{1}{W_\ell}\sum_{j \in \ell} w_j d_{ij}\\
    s_i&=&\frac{b_i - a_i}{\max(a_i, b_i)}\label{eq_clustqual_asw}
\end{eqnarray}

L'indice finalement utilisé correspond à la moyenne pondérée des silhouettes $s_i$. Cette valeur est retournée dans l'élément \code{stats}. La moyenne pondérée des silhouettes de chaque groupe est donnée dans l'élément \code{ASW} et mesure séparément la cohérence de chaque groupe.
