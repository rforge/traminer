...
Some functions of WeightedCluster use the free GraphViz \citep{Gansner99anopen}. It should be installed before launching R for these functions to work properly. You may download it here: \url{http:\\www.graphviz.org}.

The \pkg{WeightedCluster} library can then be installed and loaded using the following commands.

<<install, echo=TRUE, results="hide", eval=FALSE>>=
...
We can regroups similar sequence using hierarchical clustering with \code{"average"} method using weights (\code{aggMvad\$aggWeights}) (any method may be used).
