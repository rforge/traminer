...
opts_chunk$set(message=FALSE, prompt=TRUE, dev="pdf", echo=TRUE, comment=NA, crop=TRUE, small.mar=TRUE, fig.align="center", fig.path="graphics/WCP-", size="small")
...
<<hierclust, echo=TRUE, setwidth=50>>=
...
<<avgqualcompute, echo=TRUE, setwidth=40>>=
...
<<pamcompute, echo=TRUE, setwidth=40>>=
...
<<mppamplotd, include=FALSE>>=
pdf(file="graphics/WCP-pamclust5.pdf", height=6, width=9)
seqdplot(mvad.seq, group=pamClustRange$clustering$cluster5, border=NA)
dev.off()
@

<<mppamplot, fig.height=6, fig.width=9, eval=FALSE>>=
seqdplot(mvad.seq, group=pamClustRange$clustering$cluster5, border=NA)
@
