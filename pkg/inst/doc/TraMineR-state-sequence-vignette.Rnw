...
\author{Alexis Gabadinho\\University of Geneva \And
        ...
        Nicolas S. Müller\\University of Geneva \And
...
\Plainauthor{Alexis Gabadinho, Gilbert Ritschard, Nicolas S. Müller, Matthias Studer} %% comma-separated
...
\SweaveOpts{prefix.string=Graphiques/SW} % sets the prefix for figure files generated par Sweave
...
\raisebox{14.3cm}[0cm][0cm]{\parbox{\linewidth}{\small\normalfont This vignette is distributed with the \R package \traminer. Please, cite it as:\\[.5ex]
Gabadinho, A., G. Ritschard, N.S. Müller and M. Studer (2011). ``Analyzing and Visualizing State Sequences in \proglang{R} with \pkg{TraMineR}.'' \textit{Journal of Statistical Software}, \textbf{40}(4), 1--37. URL \url{http://www.jstatsoft.org/v40/i04}    }}
...
A first simple example of a representative sequence is the \emph{medoid} %or \emph{centrotype}
...
The command below finds and plots the smallest representative set that, with a neighborhood radius of 10\% (default \Comt{tsim} value), covers at least 25\% (default \Comt{trep} value) of the sequences in each of the two \emph{gcse5eq} groups:
%
...
\setkeys{Gin}{width=1\textwidth}
<<label=fg-seqrplot-mvad, fig=TRUE, echo=FALSE, width=10, height=5.5, cache=TRUE>>=
...
<<label=fig_cluster-mvad-om, fig=TRUE, echo=FALSE, width=8, height=6, cache=TRUE>>=
<<dendro-mvad-om>>
@
...
\bibitem[{Billari \emph{et~al.}(2006)Billari, Fürnkranz, and
  Prskawetz}]{billa:furn:prsk:2006}
Billari FC, Fürnkranz J, Prskawetz A (2006).
...
\bibitem[{Gabadinho \emph{et~al.}(2009)Gabadinho, Ritschard, Studer, and
  Müller}]{Gabadinho_et_al_2009TraMineR-UGuide}
Gabadinho A, Ritschard G, Studer M, Müller NS (2009).
...
\bibitem[{Gabadinho \emph{et~al.}(2010)Gabadinho, Ritschard, Studer, and
  Müller}]{GabadinhoRitschardStuderMuller2010EGC}
Gabadinho A, Ritschard G, Studer M, Müller NS (2010).
\newblock \enquote{Indice de complexité pour le tri et la comparaison de
...
\newblock \emph{Série des Documents de Travail du CREST 2006-01}, Institut
...
\bibitem[{Müller \emph{et~al.}(2010)Müller, Studer, Ritschard, and
  Gabadinho}]{MullerStuderRitschardGabadinho2010EGC}
Müller NS, Studer M, Ritschard G, Gabadinho A (2010).
\newblock \enquote{Extraction de règles d'association séquentielle
  à l'aide de modèles de durée.}
\newblock \emph{Revue des nouvelles technologies de l'information {RNTI}},
  \textbf{E-19}, 25--36.
...
\bibitem[{Ritschard \emph{et~al.}(2008)Ritschard, Gabadinho, Müller, and
  Studer}]{RitschardGabadinhoMullerStuder2008IJDMMM}
Ritschard G, Gabadinho A, Müller NS, Studer M (2008).
...
\bibitem[{Rohwer and Pötter(2002)}]{RohwerPotter2002}
Rohwer G, Pötter U (2002).
\newblock \enquote{\proglang{{TDA}}: Transition Data Analysis.}
\newblock \emph{Software}, Ruhr-Universität Bochum, Fakultät für
...
\bibitem[{Studer \emph{et~al.}(2010)Studer, Müller, Ritschard, and
  Gabadinho}]{StuderMullerRitschardGabadinho2010EGC}
Studer M, Müller NS, Ritschard G, Gabadinho A (2010).
\newblock \enquote{Classer, discriminer et visualiser des séquences
  d'événements.}
\newblock \emph{Revue des nouvelles technologies de l'information {RNTI}},
  \textbf{E-19}, 37--48.

\bibitem[{Studer \emph{et~al.}(2011)Studer, Ritschard, Gabadinho, and
  Müller}]{StuderRitschardGabadinhoMuller2011SMR}
