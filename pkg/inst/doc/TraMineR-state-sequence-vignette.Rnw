...
\raisebox{16cm}[0cm][0cm]{\parbox{\linewidth}{\normalsize\normalfont This vignette is distributed with the \R package \traminer. Please, cite it as:\\[1ex]
...
\bibitem[{Billari \emph{et~al.}(2006)Billari, F\"{u}rnkranz, and
...
\newblock \enquote{Extraction de r\`{e}gles d'association s\'{e}quentielle
  \`{a} l'aide de mod\`{e}les de dur\'{e}e.}
\newblock \emph{Revue des nouvelles technologies de l'information {RNTI}},
  \textbf{E-19}, 25--36.
...
\bibitem[{Rohwer and P\"{o}tter(2002)}]{RohwerPotter2002}
Rohwer G, P\"{o}tter U (2002).
