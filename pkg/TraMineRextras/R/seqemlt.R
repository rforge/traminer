...
seqemlt <- function(seqdata, a = 1, b = 1, weighted = TRUE) {
    ...
    ## La fonction freq définit la fréquende ce chaque situation (l'état indicé par
    ## le temps) Elle permet de suivre l'évolution de la fréquance de chaque état
    ## dans le temps
    freq <- function(seq) {
    ...
    ## le vecteur profil des transitions d'une situation S vers son futur Soit le
    ## taux de transition de S vers touts les siyuations futures S' Pour tous les
    ...
    ## séquence sous forme alphabetique (seq) et disjonctif complet (disj)
    ...
    ## identiques et ce au cours du temps Des futurs proches à court termes génère
    ## des des corrélations plus fortes que des futurs proches à long term (cf
    ## paramètres a et b)
...
print.emlt <- function(x, ...) {
    ...
    print(x$state)
    print(c("periode of observation : ", x$period))
    print("frequency of situations - states indexed in time -:")
    print(x$sit.freq)
    print("sit.cor : correlation between situations - mesuring proximities/links between situations")
    print("coord : rotation of sequencies - coordinates in the euclidean space - and inputs to clustering algorithms")
