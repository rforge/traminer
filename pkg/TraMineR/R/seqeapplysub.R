...
seqeapplysub <- function(subseq, method="count",constraint=NULL, rules=FALSE){
	...
	if(!rules) {
		return(.Call(TMR_tmrmatrixsubseqinseq, unlist(list(subseq$subseq)), 
					unlist(list(subseq$seqe)), as.double(c(constraint$maxGap)), 
		...
		return(.Call(TMR_tmrmatrixsubseqinseq, unlist(list(subseq$subseq)), 
					unlist(list(subseq$subseq)), as.double(c(constraint$maxGap)),
					as.double(c(constraint$windowSize)), as.double(c(constraint$ageMin)),
					as.double(c(constraint$ageMax)),as.double(c(constraint$ageMaxEnd)),
					lastparam))
	}
