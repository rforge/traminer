...
extern "C" {
	...
	SEXP cstringdistanceOOomp(SEXP Ssequences, SEXP seqdim, SEXP lenS, SEXP paramS, SEXP normS, SEXP magicIndexS, SEXP magicSeqS, SEXP disttypeS, SEXP numcpuS) {
		#ifdef NO_OMP
			return cstringdistanceOO(Ssequences, seqdim, lenS, paramS, normS, magicIndexS, magicSeqS, disttypeS);
		#else
		TMRLOG(5, "Starting cstringdistanceOOomp\n");
        //Objet R, matrice des distances (objet dist)
        //Indices, avec s pour séquences
        //longueur des séquences m=i, n=j
        //int m, n;
		// int numcpu = INTEGER(numcpuS)[0];
		// #ifdef HAVE_OPENMP 
		// #endif
		DistanceObject* distObj = new DistanceObject(magicIndexS, magicSeqS);
		...
		if(numcpu<=0 || numcpu>haveCores) numcpu=haveCores;
		omp_set_num_threads(numcpu);
		DistanceCalculator* ds_model= getDistanceCalculatorObject(Ssequences, seqdim, lenS, paramS, normS, disttypeS);
		#pragma omp parallel shared(distObj, nseq, ds_model)
		{
			DistanceCalculator* ds = ds_model->copy();
